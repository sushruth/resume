...
import { assertResumeConfig, parseResumeConfig, DEFAULT_CONFIG } from './config.types';
 ...
 */
export function loadResumeConfig(options: LoadOptions = {}): typeof DEFAULT_CONFIG | null {
    const {
    ...
    try {
        ...
        if (strict) {
            throw new Error(`Invalid resume config: ${error instanceof Error ? error.message : String(error)}`);
        }
 ...
 */
export function validateResumeConfig(config: unknown): asserts config is typeof DEFAULT_CONFIG {
